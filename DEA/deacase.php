...
session_start();
   ...
   if($_SERVER["REQUEST_METHOD"] == "POST")
   {
		 	$mycaseid = $_POST['caseid'];
	      	$mydrugname =$_POST['drugs']; 
	      	$branch=$_POST['branch'];
	      	$crid=$_POST['crid'];
	      	$badge=$_POST['badge'];
		 	$query1="Select *,C.name,O.o_name from dea_cases d, criminal C,officers O where case_no=$mycaseid and C.criminal_id=D.criminal_id and o.badge_id=d.badge_id";
		  	$query2="Select *,C.name,O.o_name from dea_cases d, criminal C,officers O where drugs_associated LIKE '%{$mydrugname}%' and C.criminal_id=D.criminal_id and o.badge_id=d.badge_id";
		  	$query3="Select * from dea_overview NATURAL JOIN locations where branch_name LIKE '%{$branch}%'";
		  	$query4="Select * from criminal where criminal_id=$crid";
		  	$query5="Select * from officers NATURAL JOIN dea_emp where badge_id=$badge";
	  		if($mycaseid)    
	  		{
		  		...
		  		{
			  		echo "<B>ID:".$row["case_no"]."<br>"."Case Name: ".$row["case_name"]."<br>"."Drugs associated: ".$row["drugs_associated"]."<br>"."Criminal ID: ".$row["criminal_id"]."<br>Criminal Name: ".$row["name"]."<br>"."Badge ID: ".$row["badge_id"]."<br>Officer Name: ".$row["o_name"]."<br></B><br>";
		  		}
